	...
	private static class ThreadName1 extends Thread {
		public void run() {
			synchronized (ObjectLock1) {
				...
				/*
				 * Adding sleep() method so that Thread 2 can lock ObjectLock2
				 */
				try {
					Thread.sleep(100);
				...
				/*
				 * Thread 1 has ObjectLock1 but waiting for ObjectLock2
				 */
	...
	private static class ThreadName2 extends Thread {
		public void run() {
			synchronized (ObjectLock2) {
				...
				/*
				 * Adding sleep() method so that Thread 1 can lock ObjectLock1
				 */
				try {
					Thread.sleep(100);
				} catch (InterruptedException e) {
				...
				/*
				 * Thread 2 has ObjectLock2 but waiting for ObjectLock1
				 */
				synchronized (ObjectLock1) {
